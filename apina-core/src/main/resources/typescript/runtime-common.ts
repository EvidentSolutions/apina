...
type TypeDescriptor = [TypeName, ...TypeDescriptor[]];
...
abstract class ApinaConfigBase {
    ...
    serialize(value: unknown, type: TypeDescriptor): any {
        return this.lookupSerializer(type).serialize(value);
    }

    deserialize(value: unknown, type: TypeDescriptor): any {
    ...
    registerClassSerializer<T = unknown>(name: TypeName, fields: Record<keyof T, TypeDescriptor>) {
        this.registerSerializer(name, nullSafeSerializer({
            serialize: o => Object.fromEntries(Object.entries(fields).map(([name, type]) => [name, this.serialize(o[name], type as TypeDescriptor)])),
            deserialize: o => Object.fromEntries(Object.entries(fields).map(([name, type]) => [name, this.deserialize(o[name], type as TypeDescriptor)])),
        }));
    }
    ...
    registerDiscriminatedUnionSerializer<T = unknown>(name: TypeName, discriminator: keyof T, types: Record<TypeName, TypeDescriptor>) {
        const self = this;
        this.registerSerializer(name, nullSafeSerializer({
            serialize(obj) {
    ...
    private lookupSerializer(type: TypeDescriptor): Serializer {
        ...
        if (serializerProvider()) {
            const args = type.slice(1) as TypeDescriptor[];
